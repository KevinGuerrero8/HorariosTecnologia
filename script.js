...
document.getElementById('eventFuture-button').addEventListener('click', () => {
            document.getElementById('calendar-container').style.display = 'block';
});

// Botón para consultar el evento seleccionado
document.getElementById('search-event-btn').addEventListener('click', () => {
    // Obtener el valor de la fecha y hora seleccionada
    const eventDateTime = document.getElementById('event-datetime').value;

    // Validar si se ha seleccionado una fecha y hora
        if (eventDateTime) {
            // Si contiene datos, buscar el evento correspondiente
            searchEventByDate(eventDateTime);
        } else {
            // Si no se selecciona nada, muestra un mensaje de advertencia
            document.getElementById('event-result').textContent = 'Por favor, selecciona una fecha y hora.';
        }
});
